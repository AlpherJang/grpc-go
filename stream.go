...
func (cs *clientStream) withRetry(op func(a *csAttempt) error, onSuccess func()) error {
	...
	for {
		...
		if err == nil || (err == io.EOF && a.s.Status().Code() == codes.OK) {
			onSuccess()
			cs.mu.Unlock()
			glog.Errorf("grpc client stream withRetry return err one: %+v", err)
			return err
		}
...
func (cs *clientStream) SendMsg(m interface{}) (err error) {
	endpoint := cs.cc.parsedTarget.Endpoint
	glog.Infof("[%s]开始调用SendMsg", endpoint)
	defer func() {
		if err != nil {
			glog.Errorf("[%s]SendMsg 报错: %s", endpoint, err.Error())
		}
	...
	glog.Infof("[%s]开始调用prepareMsg", endpoint)
	hdr, payload, data, err := prepareMsg(m, cs.codec, cs.cp, cs.comp)
	if err != nil {
		glog.Errorf("[%s]grpc client stream prepareMsg 失败:  %s", endpoint, err.Error())
		return err
	}
	...
	op := func(a *csAttempt) error {
		glog.Infof("[%s]开始调用执行csAttempt op", endpoint)
		err := a.sendMsg(m, hdr, payload, data)
		// nil out the message and uncomp when replaying; they are only needed for
		// stats which is disabled for subsequent attempts.
		m, data = nil, nil
		if err != nil {
			glog.Errorf("[%s]grpc client stream sendMsg 失败: %s", endpoint, err.Error())
		}
		return err
	}
	glog.Infof("[%s]开始调用csWithRetry", endpoint)
	err = cs.withRetry(op, func() { cs.bufferForRetryLocked(len(hdr)+len(payload), op) })
	if err != nil {
		glog.Errorf("[%s]grpc client stream retry 失败 : %s", endpoint, err.Error())
	}
